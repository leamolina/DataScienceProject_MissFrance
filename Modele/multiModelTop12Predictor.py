...
class MultiModelTop12Predictor(object):
    ...
    #Deux possibilités pour accuracy score
    # 1 : réécrire le code de evaluate_prediction pour obtenir un truc entre 0 et 1
    # 2 : utiliser les accuracy_score de mes 12 modèles (faire une moyenne ?)
    # --> Tester les 2
    ...
    def fraction_of_concordant_pairs(self, real_rank, predicted_rank):
        ...
        for i in range(1, 13):
            for j in range(i+1, 13):
                ...
                if (len(real_rank_i)>0 and len(real_rank_j)>0 and len(pred_rank_j)>0 and len(pred_rank_i)>0) and ((real_rank_i[0] > real_rank_j[0] and pred_rank_i[0] > pred_rank_j[0]) or (
