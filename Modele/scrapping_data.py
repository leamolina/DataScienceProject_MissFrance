...
def find_rank(rank):
    ...
    if len(worlds) == 0 or len(worlds) == 1:
        return np.NaN
    if worlds[0] == 'Prix':
        return np.NaN
    if worlds[0] == 'Top':
        nb = int(worlds[2].replace('(', '').replace(')', '').replace('e', ''))
        if 'dauphine)' in worlds or 'dauphine' in worlds:
            return nb + 1
        else:
            return nb
    if worlds[0] == 'Miss':
        return 1
    if len(worlds) > 0 and worlds[1] == 'dauphine':
        return int(worlds[0].replace('e', '').replace('r', '')) + 1
    else:
        return np.NaN
...
for year in range(2009, 2024):
    ...
    print('Début du scrapping de l\'année ' + str(year))
    if response.ok:
...
if page.ok:
    print('Début du scrapping de l\'année 2024')
